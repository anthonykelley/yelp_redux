...
import { Header, Grid, Card, Segment, Image, Icon, Container } from 'semantic-ui-react';
...
class Restaurants extends Component {
  ...
  rate = (restaurant, rating = 0) => {
    ...
    const query = myRating ? `?id=${myRating.id}` : '';
  ...
  loadFunc = () => {
    ...
    axios.get(`/api/restaurants?page=${this.state.page + 1}`)
      .then( res => {
        this.props.dispatch({ type: 'MORE_RESTAURANTS', restaurants: res.data.restaurants });
